...
@else
<form method="POST" action="/user/events/{{$event->id}}/edit">
@endif
{{ csrf_field() }}
...
<div class="row border-head">
    ...
    <div>
        ...
        <button type="submit" class="btn btn-primary" id="spielerstellen">Änderungen speichern</button>
    </div>
</div>
<div class="row">
<div class="col-sm-6">
  ...
  <div class="form-group">
    @if($role == 'admin' || $role == 'lieferant')
      ...
      @foreach($fields as $field)
        ...
        <input class="checkbox" type="checkbox" name="{{$field->id}}" value="1"
        @if($event->field_id == $field->id)
          checked
        @endif
        />
      @endforeach
...
<div style="margin: 0px" class="row">
  ...
  <div style="padding: 0" class="form-group col-sm-5">
    ...
    <input type="text" class="form-control" id="start" name="start" value="{{$start}}" placeholder="hh:mm">
  </div>
<div style="padding: 0; text-align:center; padding-bottom: 10px;" class="col-sm-2 event-vert">bis</div>
  <div style="padding: 0" class="form-group col-sm-5">

    <input type="text" class="form-control" id="end" name="end" value="{{$end}}" placeholder="hh:mm">
  </div>
 </div>

  <div class="form-group" id='maxplayers'>
    Maximale Teilnehmer
    <input type="number" class="form-control" id="players" name="players" max="4" placeholder="{{$event->maxplayers}}" value="{{$event->players}}">
  </div>
