...
Willkommen {{ $user_me->first_name }}! <br>

@if($users->first())

Folgende Vereine stehen dir zur Verfügung:<br><br>

@foreach ($clubs as $club)
  {{ $club->name }} <br>

  <form action="{{ url('/user/club/join', [$club->id]) }}" method="POST">

    {{ csrf_field() }}

    <input type ="submit" value="Beitreten" class="btn btn-primary">
  </form>
...
<div style="margin: 0px" class="row">
  ...
  <div style="padding: 0" class="form-group col-sm-5">
    ...
    <input type="time" class="form-control" id="start" name="start" placeholder="hh:mm">
  </div>
<div style="padding: 0; text-align:center; padding-bottom: 10px;" class="col-sm-2 event-vert">bis</div>
  <div style="padding: 0" class="form-group col-sm-5">

    <input type="time" class="form-control" id="end" name="end" placeholder="hh:mm">
  </div>
 </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    $(document).on('change', '.field', function(){
      console.log("Korrigieren sie Start- und Endzeitpunkt bitte!");
      ...
      $.ajax({
        ...
        success:function(data){
          if(start < end || end.length == 0){
            ...
            alert('Zu Diesem Zeitpunkt findet bereits eine Veranstaltung auf diesem Feld statt.');
      ...
      $.ajax({
        ...
        success:function(data){
          if(start < end || start.length == 0){
            ...
            $('#end').val('');
            alert('Korrigieren sie Start- und Endzeitpunkt bitte!');
          }
        },
        error:function(){
          console.log('error');
